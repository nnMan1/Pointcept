...
class MaskModule(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.return_attn_masks:
            attn_mask = output_mask.detach().sigmoid() < 0.5
            attn_mask.permute(1, 0)[torch.where(attn_mask.sum(0) == attn_mask.shape[0])] = False
...
class QueryRefinement(nn.Module):
    ...
    def forward(self, point_features, attn_mask, pos_encoding, offset, queries, query_pos_encoding):
        ...
        src_pcd = self.lin_squeez(point_features)
...
class EMask3D(nn.Module):
    ...
    def __init__(self, 
        ...
        self.super_point_pooling = SuperpointPooling()
        self.super_point_unpooling = SuperpointUnpooling()
        ...
        self.edges_head = nn.Sequential(
                nn.Linear(decoder['mask_modules'][0]['hidden_dim'], decoder['mask_modules'][0]['hidden_dim']), 
                nn.ReLU(),
                nn.Linear(decoder['mask_modules'][0]['hidden_dim'], 1)
            )
        ...
        self.edge_criteria = build_criteria(edges_criteria)
    ...
    def __compute_loss(self, pred, data):
        ...
        axiliary_losses['loss'] = 5 * axiliary_losses['mask_ce'] \
                                + 2 * axiliary_losses['mask_dice'] \
                                + 2 * axiliary_losses['seg_ce'] \
                                + self.edge_criteria(pred[-1]['edges'], data["border_dist"].float())
        
        return axiliary_losses

    def forward(self, data):

        data = self.super_point_pooling(data) #if seg_indices are present, it will return pooled data

        features = self.encoder(data)
       
        queries = self.query_pooling(data).squeeze(-1)
        pred = self.decoder(data, features, queries)   
        pred[-1]['edges'] = self.edges_head(features['features']).squeeze(-1)
        ...
        if not self.training:
            ...
            data['output_mask'] = masks['output_mask']#[data['seg_indices'].cpu()]
            self.super_point_unpooling(data)
            masks['output_mask'] = data['output_mask']  
            
            # masks = db_scan(data, masks)
            
            return_dict['pred_masks'], return_dict['pred_scores'], return_dict['pred_classes'] = select_masks(masks['output_mask'].cpu(), masks['output_class'].cpu(), offset=data['offset'])
            
            return_dict['pred_masks'] = return_dict['pred_masks'][0].T
