...
# PTV3-ONLY (no DINO) + SCANNET-PRETRAINED BACKBONE, merged-synthetic dataset.
# Ablation counterpart to insseg-...-VI-dinocache-amp: identical model, data,
# schedule and AMP, but the image branch is removed entirely — the encoder
# backbone is a bare PT-V3FeatureExtractor (no MergeFeatures, no
# Image2PointCLoud, no LoadImageFeatures / image_features in the pipeline).
# Isolates the contribution of the DinoV3 features.
#
# The PT-V3 backbone is initialized from the official Pointcept checkpoint
# scannet-semseg-pt-v3m1-0-base (huggingface.co/Pointcept/PointTransformerV3),
# converted by tools/… offline: keys remapped to
# module.encoder.backbone.backbone.*, seg_head dropped, and the stem conv
# dropped (ScanNet model has in_channels=6 color+normal, here 3 coord-only —
# that single layer trains from scratch). Decoder/matcher train from scratch.
...
weight = 'backbones/ptv3-scannet-semseg-base-myspformer.pth'
