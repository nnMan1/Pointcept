...
# AMP-ONLY dinocache on the MERGED-SYNTHETIC dataset.
# Ablation counterpart to insseg-...-VI-dinocache-concat-amp-norm: same
# merged-synthetic MechanicalAssemblySynth training data
# (data/segment-assembly-merged-synthetic/data) with the mech_synth DinoV3
# feature cache and enable_amp=True, but WITHOUT concat fusion and WITHOUT
# feature normalization: the DinoV3 features are fused by the default
# per-level AVERAGING (no fusion='concat', backbone_out_channels stays 64)
# and there is no proj_norm / concat_norm.
#
# Uses PRECOMPUTED DinoV3 features (no ViT at train time):
#   * model1 (Image2PointCLoud) has model_type=None: per-view patch grids come
#     from the mech_synth feature cache (run feature extraction first!).
#   * LoadImageFeatures attaches the fp16 grids; Collect ships 'image_features'.
#   * keep_on_cpu: grids stay in pinned host memory, streamed per sample.
