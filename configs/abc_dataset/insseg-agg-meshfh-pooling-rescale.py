...
# MESH-FH SUPERPOINTS + POOLING ACTIVE DURING TRAINING.
# concat-amp-norm-rescale recipe, plus:
#   * ComputeSuperpoints(mesh_fh): Felzenszwalb-Huttenlocher on the mesh
#     dual graph (libs/dl_integration), granularity randomised per sample
#     with k log-uniform in [0.005, 0.03] — measured to give a matching
#     superpoint count on CAD and on real scans once each mesh is
#     normalised to ref_diag (synthetic 9.2k vs real 10.5k sp at k=0.01),
#     so the band transfers across domains.
#   * use_superpoint_pooling=True: the model predicts per superpoint
#     instead of per point. Randomised granularity is what makes this
#     safe across the domain gap (see libs/dl_integration/README.md).
# Needs the container overlay (--overlay ...pointcept-multiview-overlay.img:ro).
# Control: same recipe without pooling, real AP50 0.2768.
...
data = dict(
    ...
    train=dict(
        ...
        transform=[
            ...
            dict(type="ComputeSuperpoints", method="mesh_fh", randomize=True,
                 k_range=(0.005, 0.03), weight_noise=0.1, data_root=data_root),
    ...
    val=dict(
        ...
        transform=[
            dict(type="LoadImageFeatures", features_root=features_root_synth),
            dict(type="CenterShift", apply_z=True),
            dict(type="ComputeSuperpoints", method="mesh_fh", k=0.01, data_root=data_root),
            dict(
                type="Copy",
                keys_dict={
                    "coord": "origin_coord",
